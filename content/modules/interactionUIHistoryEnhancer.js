  ...
  window.addEventListener('DOMContentLoaded', () => {
    ...
    const waitForInitialization = setInterval(() => {
      if (window.leadManagerPro && window.leadManagerPro.memberInteractionUI) {
        ...
        // Añadir la sección de historial al panel de interacción
        const originalCreateUI = interactionUI.createMessageUI;
        
        interactionUI.createMessageUI = function() {
          // Primero, obtener el panel original
          const container = originalCreateUI.call(this);
          
          // Intentar encontrar la sección correcta donde insertar el historial
          // Buscar la sección de tiempo de espera o las opciones avanzadas
          let insertAfterElement = container.querySelector('.lead-manager-options-advanced');
          
          // Si no encontramos las opciones avanzadas, buscar el tiempo de espera
          if (!insertAfterElement) {
            // Buscar el elemento que contiene "Tiempo de espera entre interacciones"
            const allLabels = container.querySelectorAll('label, div');
            for (const label of allLabels) {
              if (label.textContent && label.textContent.includes('Tiempo de espera')) {
                // Encontrar el elemento padre que contiene esta etiqueta y el input
                let parent = label.parentNode;
                // A veces necesitamos subir un nivel más
          ...
          // Si aún no encontramos, buscamos el elemento del acordeón
          if (!insertAfterElement) {
            const accordionElement = container.querySelector('.lead-manager-accordion');
            if (accordionElement) {
              // Buscar su contenedor padre
              insertAfterElement = accordionElement.parentNode;
            }
          }
          
          // Si todavía no encontramos, usar el último div antes de los botones
          if (!insertAfterElement) {
            const buttons = container.querySelectorAll('button');
            if (buttons.length > 0) {
              const lastButton = buttons[buttons.length - 1];
              const parentDivs = container.querySelectorAll('div');
              for (let i = parentDivs.length - 1; i >= 0; i--) {
                if (!parentDivs[i].contains(lastButton)) {
                  insertAfterElement = parentDivs[i];
                  break;
                }
              }
            }
          ...
          // Si encontramos donde insertar y no existe ya la sección de historial
          if (insertAfterElement && !container.querySelector('#lead-manager-history-container')) {
            console.log('Añadiendo sección de historial al panel de interacción...');
            ...
            // Insertar la sección de historial antes de la sección de opciones avanzadas
            advancedOptionsSection.parentNode.insertBefore(historySection, advancedOptionsSection.nextSibling);
          }
            ...
            memberInteraction.startInteraction = async function(callback) {
              ...
              return await originalStartInteractionFn.call(this, async (progress) => {
                ...
                if (progress.type === 'progress') {
                  ...
                  const interactionButton = container.querySelector('.lead-manager-interaction-button');
                  if (interactionButton) {
                    interactionButton.textContent = 'Iniciar Interacción';
                    interactionButton.style.backgroundColor = '#4267B2';
                  }
        ...
        interactionUI.show = function() {
          ...
          setTimeout(() => {
            // Intentar encontrar el panel por su ID
            const interactionPanel = document.getElementById('lead-manager-interaction-ui');
